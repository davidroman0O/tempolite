...
func (vc *VersionCreate) SetEntityID(sei schema.WorkflowEntityID) *VersionCreate {
	vc.mutation.SetEntityID(sei)
	return vc
}
...
func (vc *VersionCreate) SetData(m map[string]interface{}) *VersionCreate {
...
func (vc *VersionCreate) SetID(si schema.VersionID) *VersionCreate {
	vc.mutation.SetID(si)
	return vc
}

// SetWorkflowID sets the "workflow" edge to the WorkflowEntity entity by ID.
func (vc *VersionCreate) SetWorkflowID(id schema.WorkflowEntityID) *VersionCreate {
	vc.mutation.SetWorkflowID(id)
	return vc
}

// SetWorkflow sets the "workflow" edge to the WorkflowEntity entity.
func (vc *VersionCreate) SetWorkflow(w *WorkflowEntity) *VersionCreate {
	return vc.SetWorkflowID(w.ID)
}
...
func (vc *VersionCreate) check() error {
	if _, ok := vc.mutation.EntityID(); !ok {
		return &ValidationError{Name: "entity_id", err: errors.New(`ent: missing required field "Version.entity_id"`)}
	}
	if _, ok := vc.mutation.ChangeID(); !ok {
		return &ValidationError{Name: "change_id", err: errors.New(`ent: missing required field "Version.change_id"`)}
	}
	...
	if _, ok := vc.mutation.Data(); !ok {
		return &ValidationError{Name: "data", err: errors.New(`ent: missing required field "Version.data"`)}
	}
	if _, ok := vc.mutation.CreatedAt(); !ok {
	...
	if len(vc.mutation.WorkflowIDs()) == 0 {
		return &ValidationError{Name: "workflow", err: errors.New(`ent: missing required edge "Version.workflow"`)}
	}
	return nil
}
...
func (vc *VersionCreate) createSpec() (*Version, *sqlgraph.CreateSpec) {
	...
	if nodes := vc.mutation.WorkflowIDs(); len(nodes) > 0 {
		...
		func(i int, root context.Context) {
			...
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				...
				if specs[i].ID.Value != nil && nodes[i].ID == 0 {
					...
					nodes[i].ID = schema.VersionID(id)
				}
				mutation.done = true
				return nodes[i], nil
			})
