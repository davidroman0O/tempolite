...
func (wedq *WorkflowExecutionDataQuery) QueryExecution() *WorkflowExecutionQuery {
...
func (wedq *WorkflowExecutionDataQuery) FirstID(ctx context.Context) (id schema.WorkflowExecutionDataID, err error) {
	var ids []schema.WorkflowExecutionDataID
...
func (wedq *WorkflowExecutionDataQuery) FirstIDX(ctx context.Context) schema.WorkflowExecutionDataID {
...
func (wedq *WorkflowExecutionDataQuery) OnlyID(ctx context.Context) (id schema.WorkflowExecutionDataID, err error) {
	var ids []schema.WorkflowExecutionDataID
...
func (wedq *WorkflowExecutionDataQuery) OnlyIDX(ctx context.Context) schema.WorkflowExecutionDataID {
...
func (wedq *WorkflowExecutionDataQuery) IDs(ctx context.Context) (ids []schema.WorkflowExecutionDataID, err error) {
...
func (wedq *WorkflowExecutionDataQuery) IDsX(ctx context.Context) []schema.WorkflowExecutionDataID {
...
func (wedq *WorkflowExecutionDataQuery) Clone() *WorkflowExecutionDataQuery {
	...
	return &WorkflowExecutionDataQuery{
		...
		withExecution: wedq.withExecution.Clone(),
...
func (wedq *WorkflowExecutionDataQuery) WithExecution(opts ...func(*WorkflowExecutionQuery)) *WorkflowExecutionDataQuery {
	query := (&WorkflowExecutionClient{config: wedq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wedq.withExecution = query
	return wedq
}
...
//		GroupBy(workflowexecutiondata.FieldExecutionID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.WorkflowExecutionData.Query().
//		Select(workflowexecutiondata.FieldExecutionID).
//		Scan(ctx, &v)
...
func (wedq *WorkflowExecutionDataQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*WorkflowExecutionData, error) {
	...
	if query := wedq.withExecution; query != nil {
		if err := wedq.loadExecution(ctx, query, nodes, nil,
			func(n *WorkflowExecutionData, e *WorkflowExecution) { n.Edges.Execution = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (wedq *WorkflowExecutionDataQuery) loadExecution(ctx context.Context, query *WorkflowExecutionQuery, nodes []*WorkflowExecutionData, init func(*WorkflowExecutionData), assign func(*WorkflowExecutionData, *WorkflowExecution)) error {
	ids := make([]schema.WorkflowExecutionID, 0, len(nodes))
	nodeids := make(map[schema.WorkflowExecutionID][]*WorkflowExecutionData)
	for i := range nodes {
...
func (wedq *WorkflowExecutionDataQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := wedq.ctx.Fields; len(fields) > 0 {
		...
		if wedq.withExecution != nil {
			_spec.Node.AddColumnOnce(workflowexecutiondata.FieldExecutionID)
		}
	}
