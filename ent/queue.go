...
type Queue struct {
	config `json:"-"`
	// ID of the ent.
	ID schema.QueueID `json:"id,omitempty"`
...
func (e QueueEdges) EntitiesOrErr() ([]*WorkflowEntity, error) {
	if e.loadedTypes[0] {
	...
	for i := range columns {
		...
		case queue.FieldID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				q.ID = schema.QueueID(value.Int64)
			}
		case queue.FieldName:
...
func (q *Queue) QueryEntities() *WorkflowEntityQuery {
