...
func (aeq *ActivityExecutionQuery) QueryActivity() *ActivityEntityQuery {
	query := (&ActivityEntityClient{config: aeq.config}).Query()
...
func (aeq *ActivityExecutionQuery) FirstID(ctx context.Context) (id schema.ActivityExecutionID, err error) {
	var ids []schema.ActivityExecutionID
...
func (aeq *ActivityExecutionQuery) FirstIDX(ctx context.Context) schema.ActivityExecutionID {
...
func (aeq *ActivityExecutionQuery) OnlyID(ctx context.Context) (id schema.ActivityExecutionID, err error) {
	var ids []schema.ActivityExecutionID
...
func (aeq *ActivityExecutionQuery) OnlyIDX(ctx context.Context) schema.ActivityExecutionID {
...
func (aeq *ActivityExecutionQuery) IDs(ctx context.Context) (ids []schema.ActivityExecutionID, err error) {
...
func (aeq *ActivityExecutionQuery) IDsX(ctx context.Context) []schema.ActivityExecutionID {
	ids, err := aeq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aeq *ActivityExecutionQuery) WithActivity(opts ...func(*ActivityEntityQuery)) *ActivityExecutionQuery {
	query := (&ActivityEntityClient{config: aeq.config}).Query()
...
//		GroupBy(activityexecution.FieldActivityEntityID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ActivityExecution.Query().
//		Select(activityexecution.FieldActivityEntityID).
//		Scan(ctx, &v)
...
func (aeq *ActivityExecutionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ActivityExecution, error) {
	...
	if query := aeq.withActivity; query != nil {
		if err := aeq.loadActivity(ctx, query, nodes, nil,
			func(n *ActivityExecution, e *ActivityEntity) { n.Edges.Activity = e }); err != nil {
			return nil, err
		}
	}
...
func (aeq *ActivityExecutionQuery) loadActivity(ctx context.Context, query *ActivityEntityQuery, nodes []*ActivityExecution, init func(*ActivityExecution), assign func(*ActivityExecution, *ActivityEntity)) error {
	ids := make([]schema.ActivityEntityID, 0, len(nodes))
	nodeids := make(map[schema.ActivityEntityID][]*ActivityExecution)
	for i := range nodes {
		fk := nodes[i].ActivityEntityID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "activity_entity_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aeq *ActivityExecutionQuery) loadExecutionData(ctx context.Context, query *ActivityExecutionDataQuery, nodes []*ActivityExecution, init func(*ActivityExecution), assign func(*ActivityExecution, *ActivityExecutionData)) error {
	...
	nodeids := make(map[schema.ActivityExecutionID]*ActivityExecution)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(activityexecutiondata.FieldExecutionID)
	}
...
func (aeq *ActivityExecutionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aeq.ctx.Fields; len(fields) > 0 {
		...
		if aeq.withActivity != nil {
			_spec.Node.AddColumnOnce(activityexecution.FieldActivityEntityID)
		}
	}
