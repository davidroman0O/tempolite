...
func (qq *QueueQuery) QueryEntities() *WorkflowEntityQuery {
	query := (&WorkflowEntityClient{config: qq.config}).Query()
...
func (qq *QueueQuery) FirstID(ctx context.Context) (id schema.QueueID, err error) {
	var ids []schema.QueueID
...
func (qq *QueueQuery) FirstIDX(ctx context.Context) schema.QueueID {
...
func (qq *QueueQuery) OnlyID(ctx context.Context) (id schema.QueueID, err error) {
	var ids []schema.QueueID
...
func (qq *QueueQuery) OnlyIDX(ctx context.Context) schema.QueueID {
...
func (qq *QueueQuery) IDs(ctx context.Context) (ids []schema.QueueID, err error) {
...
func (qq *QueueQuery) IDsX(ctx context.Context) []schema.QueueID {
	ids, err := qq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (qq *QueueQuery) WithEntities(opts ...func(*WorkflowEntityQuery)) *QueueQuery {
	query := (&WorkflowEntityClient{config: qq.config}).Query()
...
func (qq *QueueQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Queue, error) {
	...
	if query := qq.withEntities; query != nil {
		if err := qq.loadEntities(ctx, query, nodes,
			func(n *Queue) { n.Edges.Entities = []*WorkflowEntity{} },
			func(n *Queue, e *WorkflowEntity) { n.Edges.Entities = append(n.Edges.Entities, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QueueQuery) loadEntities(ctx context.Context, query *WorkflowEntityQuery, nodes []*Queue, init func(*Queue), assign func(*Queue, *WorkflowEntity)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[schema.QueueID]*Queue)
	for i := range nodes {
