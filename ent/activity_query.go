...
func (aq *ActivityQuery) QueryExecutions() *ActivityExecutionQuery {
	query := (&ActivityExecutionClient{config: aq.config}).Query()
...
func (aq *ActivityQuery) WithExecutions(opts ...func(*ActivityExecutionQuery)) *ActivityQuery {
	query := (&ActivityExecutionClient{config: aq.config}).Query()
...
func (aq *ActivityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Activity, error) {
	...
	if query := aq.withExecutions; query != nil {
...
func (aq *ActivityQuery) loadExecutions(ctx context.Context, query *ActivityExecutionQuery, nodes []*Activity, init func(*Activity), assign func(*Activity, *ActivityExecution)) error {
	...
	query.Where(predicate.ActivityExecution(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(activity.ExecutionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.activity_executions
		if fk == nil {
			return fmt.Errorf(`foreign-key "activity_executions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
