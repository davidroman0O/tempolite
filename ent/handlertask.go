...
type HandlerTask struct {
	...
	TaskType handlertask.TaskType `json:"task_type,omitempty"`
...
func (*HandlerTask) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case handlertask.FieldPayload, handlertask.FieldResult, handlertask.FieldError:
			values[i] = new([]byte)
		case handlertask.FieldID, handlertask.FieldTaskType, handlertask.FieldHandlerName, handlertask.FieldStatus:
	...
	for i := range columns {
		...
		case handlertask.FieldTaskType:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				ht.TaskType = handlertask.TaskType(value.String)
			}
...
func (ht *HandlerTask) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", ht.TaskType))
	builder.WriteString(", ")
