...
func (htc *HandlerTaskCreate) SetTaskType(ht handlertask.TaskType) *HandlerTaskCreate {
	htc.mutation.SetTaskType(ht)
	return htc
}

// SetNillableTaskType sets the "task_type" field if the given value is not nil.
func (htc *HandlerTaskCreate) SetNillableTaskType(ht *handlertask.TaskType) *HandlerTaskCreate {
	if ht != nil {
		htc.SetTaskType(*ht)
	}
	return htc
...
func (htc *HandlerTaskCreate) defaults() {
	if _, ok := htc.mutation.TaskType(); !ok {
		v := handlertask.DefaultTaskType
		htc.mutation.SetTaskType(v)
	}
}
...
func (htc *HandlerTaskCreate) check() error {
	if _, ok := htc.mutation.TaskType(); !ok {
		return &ValidationError{Name: "task_type", err: errors.New(`ent: missing required field "HandlerTask.task_type"`)}
	}
	if v, ok := htc.mutation.TaskType(); ok {
...
func (htc *HandlerTaskCreate) createSpec() (*HandlerTask, *sqlgraph.CreateSpec) {
	...
	if value, ok := htc.mutation.TaskType(); ok {
