...
func (seq *SagaExecutionQuery) QuerySaga() *SagaEntityQuery {
	query := (&SagaEntityClient{config: seq.config}).Query()
...
func (seq *SagaExecutionQuery) QueryValues() *SagaValueQuery {
...
func (seq *SagaExecutionQuery) FirstID(ctx context.Context) (id schema.SagaExecutionID, err error) {
	var ids []schema.SagaExecutionID
...
func (seq *SagaExecutionQuery) FirstIDX(ctx context.Context) schema.SagaExecutionID {
...
func (seq *SagaExecutionQuery) OnlyID(ctx context.Context) (id schema.SagaExecutionID, err error) {
	var ids []schema.SagaExecutionID
...
func (seq *SagaExecutionQuery) OnlyIDX(ctx context.Context) schema.SagaExecutionID {
...
func (seq *SagaExecutionQuery) IDs(ctx context.Context) (ids []schema.SagaExecutionID, err error) {
...
func (seq *SagaExecutionQuery) IDsX(ctx context.Context) []schema.SagaExecutionID {
	ids, err := seq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (seq *SagaExecutionQuery) WithSaga(opts ...func(*SagaEntityQuery)) *SagaExecutionQuery {
	query := (&SagaEntityClient{config: seq.config}).Query()
...
func (seq *SagaExecutionQuery) WithValues(opts ...func(*SagaValueQuery)) *SagaExecutionQuery {
	query := (&SagaValueClient{config: seq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(sagaexecution.FieldSagaEntityID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.SagaExecution.Query().
//		Select(sagaexecution.FieldSagaEntityID).
//		Scan(ctx, &v)
...
func (seq *SagaExecutionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SagaExecution, error) {
	...
	if query := seq.withSaga; query != nil {
		if err := seq.loadSaga(ctx, query, nodes, nil,
			func(n *SagaExecution, e *SagaEntity) { n.Edges.Saga = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := seq.withValues; query != nil {
		if err := seq.loadValues(ctx, query, nodes,
			func(n *SagaExecution) { n.Edges.Values = []*SagaValue{} },
			func(n *SagaExecution, e *SagaValue) { n.Edges.Values = append(n.Edges.Values, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (seq *SagaExecutionQuery) loadSaga(ctx context.Context, query *SagaEntityQuery, nodes []*SagaExecution, init func(*SagaExecution), assign func(*SagaExecution, *SagaEntity)) error {
	ids := make([]schema.SagaEntityID, 0, len(nodes))
	nodeids := make(map[schema.SagaEntityID][]*SagaExecution)
	for i := range nodes {
		fk := nodes[i].SagaEntityID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "saga_entity_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (seq *SagaExecutionQuery) loadExecutionData(ctx context.Context, query *SagaExecutionDataQuery, nodes []*SagaExecution, init func(*SagaExecution), assign func(*SagaExecution, *SagaExecutionData)) error {
	...
	nodeids := make(map[schema.SagaExecutionID]*SagaExecution)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(sagaexecutiondata.FieldExecutionID)
	}
...
func (seq *SagaExecutionQuery) loadValues(ctx context.Context, query *SagaValueQuery, nodes []*SagaExecution, init func(*SagaExecution), assign func(*SagaExecution, *SagaValue)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[schema.SagaExecutionID]*SagaExecution)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(sagavalue.FieldSagaExecutionID)
	}
	query.Where(predicate.SagaValue(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(sagaexecution.ValuesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.SagaExecutionID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "saga_execution_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (seq *SagaExecutionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := seq.ctx.Fields; len(fields) > 0 {
		...
		if seq.withSaga != nil {
			_spec.Node.AddColumnOnce(sagaexecution.FieldSagaEntityID)
		}
	}
