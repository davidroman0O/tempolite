...
func (weq *WorkflowExecutionQuery) QueryWorkflow() *WorkflowEntityQuery {
	query := (&WorkflowEntityClient{config: weq.config}).Query()
...
func (weq *WorkflowExecutionQuery) QueryEvents() *EventLogQuery {
...
func (weq *WorkflowExecutionQuery) FirstID(ctx context.Context) (id schema.WorkflowExecutionID, err error) {
	var ids []schema.WorkflowExecutionID
...
func (weq *WorkflowExecutionQuery) FirstIDX(ctx context.Context) schema.WorkflowExecutionID {
...
func (weq *WorkflowExecutionQuery) OnlyID(ctx context.Context) (id schema.WorkflowExecutionID, err error) {
	var ids []schema.WorkflowExecutionID
...
func (weq *WorkflowExecutionQuery) OnlyIDX(ctx context.Context) schema.WorkflowExecutionID {
...
func (weq *WorkflowExecutionQuery) IDs(ctx context.Context) (ids []schema.WorkflowExecutionID, err error) {
...
func (weq *WorkflowExecutionQuery) IDsX(ctx context.Context) []schema.WorkflowExecutionID {
	ids, err := weq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (weq *WorkflowExecutionQuery) WithWorkflow(opts ...func(*WorkflowEntityQuery)) *WorkflowExecutionQuery {
	query := (&WorkflowEntityClient{config: weq.config}).Query()
...
func (weq *WorkflowExecutionQuery) WithEvents(opts ...func(*EventLogQuery)) *WorkflowExecutionQuery {
	query := (&EventLogClient{config: weq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(workflowexecution.FieldWorkflowEntityID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.WorkflowExecution.Query().
//		Select(workflowexecution.FieldWorkflowEntityID).
//		Scan(ctx, &v)
...
func (weq *WorkflowExecutionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*WorkflowExecution, error) {
	...
	if query := weq.withWorkflow; query != nil {
		if err := weq.loadWorkflow(ctx, query, nodes, nil,
			func(n *WorkflowExecution, e *WorkflowEntity) { n.Edges.Workflow = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := weq.withEvents; query != nil {
		if err := weq.loadEvents(ctx, query, nodes,
			func(n *WorkflowExecution) { n.Edges.Events = []*EventLog{} },
			func(n *WorkflowExecution, e *EventLog) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (weq *WorkflowExecutionQuery) loadWorkflow(ctx context.Context, query *WorkflowEntityQuery, nodes []*WorkflowExecution, init func(*WorkflowExecution), assign func(*WorkflowExecution, *WorkflowEntity)) error {
	ids := make([]schema.WorkflowEntityID, 0, len(nodes))
	nodeids := make(map[schema.WorkflowEntityID][]*WorkflowExecution)
	for i := range nodes {
		fk := nodes[i].WorkflowEntityID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "workflow_entity_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (weq *WorkflowExecutionQuery) loadExecutionData(ctx context.Context, query *WorkflowExecutionDataQuery, nodes []*WorkflowExecution, init func(*WorkflowExecution), assign func(*WorkflowExecution, *WorkflowExecutionData)) error {
	...
	nodeids := make(map[schema.WorkflowExecutionID]*WorkflowExecution)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workflowexecutiondata.FieldExecutionID)
	}
...
func (weq *WorkflowExecutionQuery) loadEvents(ctx context.Context, query *EventLogQuery, nodes []*WorkflowExecution, init func(*WorkflowExecution), assign func(*WorkflowExecution, *EventLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[schema.WorkflowExecutionID]*WorkflowExecution)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(eventlog.FieldWorkflowExecutionID)
	}
	query.Where(predicate.EventLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflowexecution.EventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (weq *WorkflowExecutionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := weq.ctx.Fields; len(fields) > 0 {
		...
		if weq.withWorkflow != nil {
			_spec.Node.AddColumnOnce(workflowexecution.FieldWorkflowEntityID)
		}
	}
