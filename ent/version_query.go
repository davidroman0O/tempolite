...
func (vq *VersionQuery) QueryWorkflow() *WorkflowEntityQuery {
	query := (&WorkflowEntityClient{config: vq.config}).Query()
...
func (vq *VersionQuery) FirstID(ctx context.Context) (id schema.VersionID, err error) {
	var ids []schema.VersionID
...
func (vq *VersionQuery) FirstIDX(ctx context.Context) schema.VersionID {
...
func (vq *VersionQuery) OnlyID(ctx context.Context) (id schema.VersionID, err error) {
	var ids []schema.VersionID
...
func (vq *VersionQuery) OnlyIDX(ctx context.Context) schema.VersionID {
...
func (vq *VersionQuery) IDs(ctx context.Context) (ids []schema.VersionID, err error) {
...
func (vq *VersionQuery) IDsX(ctx context.Context) []schema.VersionID {
	ids, err := vq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (vq *VersionQuery) Clone() *VersionQuery {
	...
	return &VersionQuery{
		...
		withWorkflow: vq.withWorkflow.Clone(),
...
func (vq *VersionQuery) WithWorkflow(opts ...func(*WorkflowEntityQuery)) *VersionQuery {
	query := (&WorkflowEntityClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withWorkflow = query
	return vq
}
...
func (vq *VersionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Version, error) {
	...
	if query := vq.withWorkflow; query != nil {
		if err := vq.loadWorkflow(ctx, query, nodes, nil,
			func(n *Version, e *WorkflowEntity) { n.Edges.Workflow = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VersionQuery) loadWorkflow(ctx context.Context, query *WorkflowEntityQuery, nodes []*Version, init func(*Version), assign func(*Version, *WorkflowEntity)) error {
	ids := make([]schema.WorkflowEntityID, 0, len(nodes))
	nodeids := make(map[schema.WorkflowEntityID][]*Version)
	for i := range nodes {
		fk := nodes[i].EntityID
...
func (vq *VersionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := vq.ctx.Fields; len(fields) > 0 {
		...
		if vq.withWorkflow != nil {
			_spec.Node.AddColumnOnce(version.FieldEntityID)
		}
	}
