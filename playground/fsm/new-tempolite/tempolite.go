...
func (ai *ActivityInstance) executeWithRetry() error {
	...
	for attempt = ai.entity.RetryState.Attempts + 1; attempt <= maxAttempts; attempt++ {
		...
		if err == nil {
			...
			ai.orchestrator.db.UpdateExecutionStatus(execution.ID, ExecutionStatusCompleted)
			ai.orchestrator.db.UpdateExecutionCompletedAt(execution.ID, completedAt)
			...
			ai.err = err
			ai.orchestrator.db.UpdateExecutionStatus(execution.ID, ExecutionStatusFailed)
			ai.orchestrator.db.UpdateExecutionCompletedAt(execution.ID, completedAt)
			ai.orchestrator.db.UpdateExecutionError(execution.ID, err.Error())
...
func (ai *ActivityInstance) runActivity(execution *Execution) error {
	...
	if errInterface != nil {
		...
		ai.orchestrator.db.UpdateExecutionError(execution.ID, errorMessage)
		...
		ai.orchestrator.db.UpdateExecutionActivityExecutionData(execution.ID, activityExecutionData)
...
func (ctx WorkflowContext) SideEffect(stepID string, sideEffectFunc interface{}) *RuntimeFuture {
	...
	// Side Effects are anonymous functions, we don't need to register them at all. We create a HandlerInfo for serialization purposes.
	handler := HandlerInfo{
...
func (si *SagaInstance) executeCompensations() {
	...
	si.orchestrator.db.UpdateExecutionStatus(si.execution.ID, ExecutionStatusFailed)
	si.orchestrator.db.UpdateExecutionError(si.execution.ID, si.err.Error())
...
func (si *SagaInstance) onCompleted(_ context.Context, _ ...interface{}) error {
	...
	completedAt := time.Now()
	si.orchestrator.db.UpdateExecutionStatus(si.execution.ID, ExecutionStatusCompleted)
	si.orchestrator.db.UpdateExecutionCompletedAt(si.execution.ID, completedAt)
...
func (sei *SideEffectInstance) executeWithRetry() error {
	...
	for attempt = sei.entity.RetryState.Attempts + 1; attempt <= maxAttempts; attempt++ {
		...
		if err == nil {
			// Success
			completedAt := time.Now()
			sei.orchestrator.db.UpdateExecutionStatus(execution.ID, ExecutionStatusCompleted)
			sei.orchestrator.db.UpdateExecutionCompletedAt(execution.ID, completedAt)
			...
			completedAt := time.Now()
			sei.orchestrator.db.UpdateExecutionStatus(execution.ID, ExecutionStatusFailed)
			sei.orchestrator.db.UpdateExecutionCompletedAt(execution.ID, completedAt)
			sei.orchestrator.db.UpdateExecutionError(execution.ID, err.Error())
			sei.err = err
			if attempt < maxAttempts {
...
func (sei *SideEffectInstance) runSideEffect(execution *Execution) error {
	...
	// Update the execution with the execution data
	sei.orchestrator.db.UpdateExecutionSideEffectExecutionData(execution.ID, sideEffectExecutionData)
...
func (wi *WorkflowInstance) executeWithRetry() error {
	...
	for {
		...
		if err == nil {
			...
			wi.orchestrator.db.UpdateExecutionStatus(execution.ID, ExecutionStatusCompleted)
			wi.orchestrator.db.UpdateExecutionCompletedAt(execution.ID, completedAt)
			...
			execution.Error = err.Error()
			wi.err = err
			wi.orchestrator.db.UpdateExecutionStatus(execution.ID, ExecutionStatusFailed)
			wi.orchestrator.db.UpdateExecutionCompletedAt(execution.ID, completedAt)
			wi.orchestrator.db.UpdateExecutionError(execution.ID, err.Error())
			wi.mu.Unlock()
...
func (wi *WorkflowInstance) runWorkflow(execution *Execution) error {
	...
	if errInterface != nil {
		if continueErr, ok := errInterface.(*ContinueAsNewError); ok {
			...
			// Update execution error
			wi.orchestrator.db.UpdateExecutionError(execution.ID, errorMessage)
			...
			// Update the execution with the execution data
			wi.orchestrator.db.UpdateExecutionWorkflowExecutionData(execution.ID, workflowExecutionData)
...
func (wi *WorkflowInstance) onCompleted(_ context.Context, _ ...interface{}) error {
	...
	if wi.continueAsNew != nil {
		...
		if isRootWorkflow {
			...
			completedAt := time.Now()
			o.db.UpdateExecutionStatus(wi.execution.ID, ExecutionStatusCompleted)
			o.db.UpdateExecutionCompletedAt(wi.execution.ID, completedAt)
