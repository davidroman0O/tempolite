...
func main() {
	...
	tp, err := tempolite.New(
		...
		tempolite.NewRegistry().
			...
			Build(),
		tempolite.WithPath("./db/tempolite-zero.db"),
		tempolite.WithDestructive(),
		tempolite.WithWorkerConfig(tempolite.WorkerConfig{
			InitialWorkflowsWorkers:    0,
			InitialActivityWorkers:     0,
			InitialSideEffectWorkers:   0,
			InitialTransctionWorkers:   0,
			InitialCompensationWorkers: 0,
		}),
	)
	if err != nil {
		log.Fatal(err)
