...
func TestUnitPrepareRootWorkflowEntityPanic(t *testing.T) {
	...
	for _, v := range execs {
		...
		if v.StackTrace == nil {
			t.Fatalf("expected stack trace, got nil")
		} else if len(*v.StackTrace) == 0 {
...
func TestUnitPrepareRootWorkflowActivityEntityWithOuputs(t *testing.T) {
	...
	workflowData, err := db.GetWorkflowExecutionDataByExecutionID(future.WorkflowExecutionID())
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnitSequentialSagasWithManualCompensation(t *testing.T) {
	...
	wrfl := func(ctx WorkflowContext) error {
		...
		saga1ID = future.EntityID().(SagaEntityID)
		if err := future.Get(); err != nil {
			return err
		}
...
func TestUnitPrepareRootWorkflowSubWorkflowEntity(t *testing.T) {
	...
	hierarchies, err := db.GetHierarchiesByParentEntityAndStep(int(rootWorkflow.ID), "sub", EntityWorkflow)
	if err != nil {
		t.Fatal(err)
	}
	...
	if hierarchy.ParentEntityID != int(rootWorkflow.ID) {
		t.Fatalf("expected parent entity ID %d, got %d", rootWorkflow.ID, hierarchy.ParentEntityID)
	}

	// Get and verify subworkflow
	subWorkflow, err := db.GetWorkflowEntity(WorkflowEntityID(hierarchy.ChildEntityID))
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnitSubWorkflowRetrySuccess(t *testing.T) {
	...
	if hierarchy.ChildExecutionID != int(subExecs[1].ID) {
...
func TestUnitSubWorkflowPanic(t *testing.T) {
	...
	// Get and verify subworkflow
	subWorkflow, err := db.GetWorkflowEntity(WorkflowEntityID(hierarchies[0].ChildEntityID))
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnitSubWorkflowPanicWithRetrySuccess(t *testing.T) {
	...
	if hierarchies[0].ChildExecutionID != int(secondExec.ID) {
