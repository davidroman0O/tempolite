			...
			if value, ok = i.tp.activities.Load(HandlerIdentity(activityEntity.Identity)); ok {
				...
				case activity.StatusCompleted, activity.StatusFailed, activity.StatusCancelled:
					...
					case activityexecution.StatusCompleted:
						// outputs, err := i.tp.convertOutputs(HandlerInfo(activityHandlerInfo), latestExec.Output)
						deserializedOutput, err := i.tp.convertOutputsFromSerialization(HandlerInfo(activityHandlerInfo), latestExec.Output)
