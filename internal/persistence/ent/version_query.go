...
func (vq *VersionQuery) QueryEntity() *EntityQuery {
	query := (&EntityClient{config: vq.config}).Query()
...
func (vq *VersionQuery) Clone() *VersionQuery {
	...
	return &VersionQuery{
		...
		withEntity: vq.withEntity.Clone(),
...
func (vq *VersionQuery) WithEntity(opts ...func(*EntityQuery)) *VersionQuery {
	query := (&EntityClient{config: vq.config}).Query()
...
func (vq *VersionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Version, error) {
	...
	if query := vq.withEntity; query != nil {
		if err := vq.loadEntity(ctx, query, nodes, nil,
			func(n *Version, e *Entity) { n.Edges.Entity = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VersionQuery) loadEntity(ctx context.Context, query *EntityQuery, nodes []*Version, init func(*Version), assign func(*Version, *Entity)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Version)
	for i := range nodes {
		if nodes[i].entity_versions == nil {
			continue
		}
		fk := *nodes[i].entity_versions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "entity_versions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
