...
func (aedq *ActivityExecutionDataQuery) QueryActivityExecution() *ActivityExecutionQuery {
...
func (aedq *ActivityExecutionDataQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (aedq *ActivityExecutionDataQuery) FirstIDX(ctx context.Context) int {
...
func (aedq *ActivityExecutionDataQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (aedq *ActivityExecutionDataQuery) OnlyIDX(ctx context.Context) int {
...
func (aedq *ActivityExecutionDataQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (aedq *ActivityExecutionDataQuery) IDsX(ctx context.Context) []int {
...
func (aedq *ActivityExecutionDataQuery) Clone() *ActivityExecutionDataQuery {
	...
	return &ActivityExecutionDataQuery{
		...
		withActivityExecution: aedq.withActivityExecution.Clone(),
...
func (aedq *ActivityExecutionDataQuery) WithActivityExecution(opts ...func(*ActivityExecutionQuery)) *ActivityExecutionDataQuery {
	query := (&ActivityExecutionClient{config: aedq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aedq.withActivityExecution = query
	return aedq
}
...
func (aedq *ActivityExecutionDataQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ActivityExecutionData, error) {
	var (
		...
		withFKs     = aedq.withFKs
		_spec       = aedq.querySpec()
		loadedTypes = [1]bool{
			aedq.withActivityExecution != nil,
		}
	)
	if aedq.withActivityExecution != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, activityexecutiondata.ForeignKeys...)
	}
	...
	if query := aedq.withActivityExecution; query != nil {
		if err := aedq.loadActivityExecution(ctx, query, nodes, nil,
			func(n *ActivityExecutionData, e *ActivityExecution) { n.Edges.ActivityExecution = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aedq *ActivityExecutionDataQuery) loadActivityExecution(ctx context.Context, query *ActivityExecutionQuery, nodes []*ActivityExecutionData, init func(*ActivityExecutionData), assign func(*ActivityExecutionData, *ActivityExecution)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*ActivityExecutionData)
	for i := range nodes {
		if nodes[i].activity_execution_execution_data == nil {
			continue
		}
		fk := *nodes[i].activity_execution_execution_data
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "activity_execution_execution_data" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
