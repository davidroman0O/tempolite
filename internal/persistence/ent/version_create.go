...
func (vc *VersionCreate) SetEntityID(id int) *VersionCreate {
	vc.mutation.SetEntityID(id)
	return vc
}

// SetEntity sets the "entity" edge to the Entity entity.
func (vc *VersionCreate) SetEntity(e *Entity) *VersionCreate {
	return vc.SetEntityID(e.ID)
}
...
func (vc *VersionCreate) check() error {
	...
	if len(vc.mutation.EntityIDs()) == 0 {
		return &ValidationError{Name: "entity", err: errors.New(`ent: missing required edge "Version.entity"`)}
	}
	return nil
}
...
func (vc *VersionCreate) createSpec() (*Version, *sqlgraph.CreateSpec) {
	...
	if nodes := vc.mutation.EntityIDs(); len(nodes) > 0 {
