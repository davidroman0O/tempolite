...
func (e VersionEdges) EntityOrErr() (*Entity, error) {
	if e.Entity != nil {
		return e.Entity, nil
...
func (v *Version) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case version.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field entity_versions", value)
			} else if value.Valid {
				v.entity_versions = new(int)
				*v.entity_versions = int(value.Int64)
			}
		default:
...
// QueryEntity queries the "entity" edge of the Version entity.
func (v *Version) QueryEntity() *EntityQuery {
	return NewVersionClient(v.config).QueryEntity(v)
}
